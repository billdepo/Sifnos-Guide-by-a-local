...
- **Never change**: `id`, `lat`, `lng`, `mapsUrl`, `url`, `month`, `day`, `endDay`, `rating`, `icon`, `access` values (`car`/`hike`/`boat`/`walking` are keys, not display text), `shelteredFrom` (compass letters), `heroImageUrl`, calendar `year`, embedded HTML attributes (`class`, `data-target` in card-link anchors — translate only the link's visible text).
- **`id` is a permalink.** It appears in URLs (`#/beaches/sandy/glyfo`) and must be byte-identical in both files. New items get one from `node scripts/assign-ids.mjs` — never hand-write or rename one.
- **`image` objects**: copy verbatim. `src`, `credit` and `creditUrl` are licence attribution, not prose — do not translate the photographer's name or the licence code.
...
- **App-shell strings** (`ui.home`, `more`, `back`, `close`, `menu`, `photoCredit`, `exploreLabel`, `todayLabel`, `todayEventLabel`, `nextEventLabel`, `noEventLabel`, `seeAll`, `searchTitle`, `searchHint`, `searchEmpty`, and the `collections`, `filters`, `map` objects): translate all display text. Keep the `{n}` and `{q}` placeholders exactly — they are substituted at runtime and a missing one renders as literal text.
