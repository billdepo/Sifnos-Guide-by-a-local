...
function renderAccessBadges(arr, labels) {
  if (!arr || !arr.length) return '';
  return arr.map(a => `<span class="access access--${a}">${labels[a] || a}</span>`).join('');
}
...
function renderNav(nav, ui) {
  const linksHTML = Object.entries(nav)
    .map(([id, label]) => `<a class="nav-link" href="#${id}" role="listitem">${label}</a>`)
    .join('');
...
function drinkCard(item, ui) {
  ...
  const mustTry = item.mustTry && item.mustTry.length
    ? `<div class="must-try">
